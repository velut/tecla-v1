...
func (Static) generateInfo() {
	fmt.Println("Generating static data for info package...")
	mg.Deps(Static.generateLicenseInfo, Static.generateGitInfo)
}

func (Static) generateLicenseInfo() error {
	fmt.Println("Generating license data for info package...")
	return esc(
...
func (Static) generateGitInfo() error {
	fmt.Println("Generating git data for info package...")

	file, err := os.Create("./static/info/gitInfo.go")
	if err != nil {
		return err
	}
	defer file.Close()

	contents := `// Code generated by magefile/generateGitInfo. DO NOT EDIT.
package info
...
`
	_, err = fmt.Fprintf(file, contents, gitTag(), gitCommit())
	return err
}
...
func (Clean) static() error {
	as := []string{
		...
		filepath.Join(staticDir, "info", "gitInfo.go"),
